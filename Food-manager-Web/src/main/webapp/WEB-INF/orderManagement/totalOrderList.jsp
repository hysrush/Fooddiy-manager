...
<body>
			...
			<div class="row wrapper border-bottom white-bg page-heading">
				<div class="col-lg-10">
					...
					<ol class="breadcrumb">
						...
						<li><a>주문관리</a></li>
						<li class="active"><strong>전체주문내역</strong></li>
					</ol>
				</div>
												...
												<div class="col-sm-4" style="padding-top: 15px">
													<div data-toggle="buttons" class="btn-group">
														<form class="search-form" method="post" action="#">
															<input type="hidden" class="input-type" name="type" /> <input type="hidden" class="input-start" name="date_start" /> <input type="hidden" class="input-end" name="date_end" />
															...
															<button type="submit" class="btn btn-outline btn-primary month-button" onclick="search('M')">한달</button>
									...
									<table class="footable table table-stripped toggle-arrow-tiny dataTables-example" data-page-size="3000">
										...
										<tbody class="todayOrderList">
											...
											<c:forEach items="${ orderList }" var="order">
												...
												<tr>
													<td class="convType orderNumber" width="100px;">${ order.no }</td>

													<td>${ order.regDate }</td>
													<td><a onclick="modalFunc(${ order.no })"> <c:forEach items="${  order.detailOrderList }" var="oneOrder" varStatus="status">
															${ oneOrder.name }
	...
	<script>
	    ...
	    function setHiddenInput(type, dateStart, dateEnd) {
	      	$('.input-type').val(type);
	   		$('.input-start').val(dateStart);
	    	$('.input-end').val(dateEnd);
	    ...
	    function search(type) {
			var today = getToday();
	       	...
	       	if(type == "T") {
	       		setHiddenInput('T', today, today);
	       	}else if(type == "W") {
	       		//일주일전 날짜 구하기
	       		date.setDate(date.getDate() - 7);
	       		date = dateConvertFormat(date);
					
	       		setHiddenInput('W', date, today);
	        }else if(type == "M"){
        ...
        $(document).ready(function() {
			...
			//날짜 선택시 포커스 효과
			if('${type}' == 'T') {
				setButtonFocus($('.today-button'));
			}else if('${type}' == 'W'){
				setButtonFocus($('.week-button'));
			}else if('${type}' == 'M'){
				setButtonFocus($('.month-button'));
			}
			
            //날짜 선택시 날짜 설정
            if('${date.dateStart}') {
				$('#date_start').attr('value','${date.dateStart}');
				$('#date_end').attr('value', '${date.dateEnd}');
            }	
